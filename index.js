...
// =====================================================
//  AETHER SCAN API  v2.1  -  cobertura amplia + never-drop-fresh
// =====================================================
//  Cambios (investigados) vs la version FIFO original:
//   - Orden de entrega CONFIGURABLE (SERVE_NEWEST_FIRST). Default FIFO = cobertura
//     amplia: los Secrets suelen estar en servers ESTABLECIDOS, no en 1/8 vacios.a1s
...
// =====================================================
// --- ORDEN DE ENTREGA A LOS BOTS ---
// false = FIFO (cobertura amplia: variedad de servers, incluidos los ESTABLECIDOS
//         donde aparecen los Secrets). <- recomendado, confirmado en vivo.
// true  = LIFO (el mas nuevo primero). Probado: confinaba los bots a los 1/8
//         vacios -> menos hallazgos. Dejalo en false salvo que pruebes lo contrario.
const SERVE_NEWEST_FIRST = false;

// Cache moderado: con FIFO, mas chico = servers servidos mas frescos (menos cola vieja),
// pero suficiente para que ~800 bots nunca queden sin server. Tunealo si ves nulls.
...
function addRetry(id) {                     // reintento de baja prioridad -> fondo, SOLO si hay sitio
    ...
    if (SERVE_NEWEST_FIRST) cache.unshift(id);   // LIFO: al fondo (se sirve al final)
    else cache.push(id);                          // FIFO: al final de la cola
    cacheSet.add(id);
    return true;
}
function serveOne() {                       // bot pide un server
    const id = SERVE_NEWEST_FIRST ? cache.pop() : cache.shift();   // LIFO vs FIFO
...
app.listen(PORT, () => {
    ...
    console.log(`📦 CACHE_LIMIT: ${CACHE_LIMIT} | orden: ${SERVE_NEWEST_FIRST ? 'LIFO (mas nuevo)' : 'FIFO (cobertura amplia)'} | nunca descarta frescos`);
    for (let i = 1; i <= 6; i++) {
