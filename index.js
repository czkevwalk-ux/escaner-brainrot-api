...
// =====================================================
// 🔔 RUTA: NOTIFICAR HALLAZGO → DISCORD (Components V2)
// =====================================================
app.post('/notify', async (req, res) => {
    ...
    try {
        // payload llega como string JSON serializado desde Lua
        // lo parseamos para enviarlo correctamente
        let parsedPayload;
        ...
        // Enviar con ?with_components=true para Components V2
        await axios.post(webhook + '?with_components=true', parsedPayload, {
            headers: { 'Content-Type': 'application/json' }
        });

        console.log(`📨 Enviado (Components V2) → ${vps_name}`);
        res.json({ status: "ok" });
    } catch (e) {
        console.log(`❌ Error Discord: ${e.message}`);
        // Si falla Components V2, intentar como embed normal (fallback)
        try {
            let parsedPayload = typeof payload === 'string' ? JSON.parse(payload) : payload;
            await axios.post(webhook, parsedPayload);
            console.log(`📨 Enviado (fallback embed) → ${vps_name}`);
            res.json({ status: "ok_fallback" });
        } catch (e2) {
            console.log(`❌ Error fallback: ${e2.message}`);
            res.json({ status: "error", reason: e2.message });
        }
    }
});
